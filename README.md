...
검색어를 입력하면 네이버·다음 뉴스 검색결과를 긁어와 중복을 합치고
관련도순으로 보여주는 사내용 웹앱입니다.
공유 계정 하나로 로그인해야 사용할 수 있습니다. 포털 API 키는 필요 없습니다.

## 구조

- `lib/collectors/` — 포털별 수집기(`naver.ts`, `daum.ts`)와 이를 병렬로 호출하는
  오케스트레이터(`index.ts`의 `collectAll`). 한 포털이 실패해도 나머지 결과는
  그대로 반환한다(`Promise.allSettled`). 각 포털을 10페이지까지 가져온다.
- `lib/merge.ts` — 여러 포털 결과를 URL·제목 기준으로 중복 제거하고
  포털이 매긴 관련도 순서를 유지한다(`mergeArticles`). 네이버를 10페이지까지
  가져오므로 페이지 간 중복 제거가 여기서 이뤄진다.
...
- `lib/selection.ts` — 엑셀에 담을 기사 고르기(순수 로직). 기본이 '전부 포함'이라
  체크된 id가 아니라 **체크를 푼 id**만 들고 다닌다.
...
- 네이버 뉴스는 **검색결과를 파싱한다**. 공식 검색 API는
  ...
  (`s.search.naver.com/p/newssearch/3/api/tab/more`, `sort=0`이 정확도순이며
  네이버 뉴스 검색의 기본값이다)를
...
- 다음 뉴스도 공식 API가 없어 검색결과 HTML을 파싱한다. 셀렉터 근거와 실제 마크업
  구조는 `lib/collectors/daum.ts` 주석에 있고, `tests/daum.test.ts`에 시각·언론사
  셀렉터가 폴백값으로 조용히 넘어가면 실패하는 회귀 테스트가 있다.
- 구글 수집기는 2026-09-03에 제거했다. Cloudflare IP에서 503으로 막혀 매 검색마다
  실패 배너만 띄웠다(로컬에서는 잘 됐다 — 데이터센터 IP가 걸러진다). 되살리려면
  `lib/collectors/`에 수집기를 다시 만들고 `index.ts`의 `DEFAULT_COLLECTORS`에
  끼우면 된다. 지운 코드는 커밋 `dd77344` 이전에서 찾을 수 있다.
...
- 기사마다 체크박스가 있고, 체크를 풀면 그 기사는 엑셀에서 빠진다(목록 위
  '전체 선택'으로 한꺼번에 켜고 끌 수 있다). 전체 선택·해제는 **지금 화면에
  보이는** 기사만 건드린다 — 기간 필터에 가려진 기사까지 손대면 필터를 되돌렸을
  때 만든 적 없는 선택이 튀어나온다. 새로 검색하면 다시 전부 체크된 상태로 돌아간다.
- 엑셀 다운로드는 화면에 보이면서 체크가 살아 있는 기사만 담고, 열은
