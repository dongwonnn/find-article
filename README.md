...
검색어를 입력하면 네이버 뉴스 검색결과를 긁어와 중복을 합치고 관련도순으로
보여주는 사내용 웹앱입니다.
...
- 다음·구글 수집기는 2026-09-03에 제거했다. 다음은 정상 동작했지만 네이버만으로
  충분하다는 판단이었고, 구글은 Cloudflare IP에서 503으로 막혀 매 검색마다 실패
  배너만 띄웠다(로컬에서는 잘 됐다 — 데이터센터 IP가 걸러진다). 되살리려면
  `lib/collectors/`에 수집기를 다시 만들고 `index.ts`의 `DEFAULT_COLLECTORS`에
  끼우면 된다. `Portal` 타입·`failedPortals`·중복 병합·포털 배지는 그대로 남아 있다.
  지운 코드는 커밋 `dd77344` 이전에서 찾을 수 있다.
